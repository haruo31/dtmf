...
$(function(){
	...
	$("body").bind("code",function(e,n){
		$("#input").val($("#input").val() + n).trigger("change");
		check_input();
		
	});

	$(".clearButton").click(function(e){
		$("#input,#result,#history").val("").trigger("change");
		e.preventDefault();
	}).trigger("change");
...
function loadSend(){
	...
	$.getScript("lib/send/tone.js?v4").done(function(){
		tones = initTones();
...
function resetRecvBuffer(){
	if($("#input").val()){
		$("#history").val( $("#input").val() + "\n" + $("#history").val());
		$("#input").val("");
	}
}

function check_input(){
	if($("#input").val().match(/0000$/)){
...
function convertBin2Text(){
	var bin = $("#input").val();
	    bin = bin.replace(/^\s+|0000$/,"");
	    bin = bin.replace(/\*/g,"e");
	    bin = bin.replace(/#/g,"f");

	console.log("[" + bin + "]");

	var utf8 = utf8_hex_string_to_string(bin);
